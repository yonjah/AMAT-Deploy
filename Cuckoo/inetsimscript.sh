#!/bin/bash
sudo wget http://www.inetsim.org/downloads/inetsim-1.2.7.tar.gz 
sudo tar -xvzf inetsim-1.2.7.tar.gz
sudo mv inetsim-1.2.7 /home/cuckoo/
sudo rm /home/cuckoo/inetsim-1.2.7/conf/inetsim.conf
groupadd inetsim


sudo apt-get install libnet-server-perl -y
sudo apt-get install libnet-dns-perl -y
sudo apt-get install libipc-shareable-perl -y 
sudo apt-get install libdigest-sha-perl -y
sudo apt-get install libio-socket-ssl-perl -y


sudo cat <<EOT >> inetsim.conf
#############################################################
#
...
# Default: inetsim.org
#
dns_default_domainname		malwaretest.lab
...
sudo mv inetsim.conf /home/cuckoo/inetsim-1.2.7/conf/

sudo rm inetsim-1.2.7.tar.gz
...
if [ ! -f inetsim-1.2.7/data/certs/default_key.pem ]; then
  if openssl version >/dev/null 2>&1; then
    echo -n "Creating default SSL key and certificate... "
    openssl req -new -x509 -days 3650 -nodes -sha1 -keyout inetsim-1.2.7/data/certs/default_key.pem -out inetsim-1.2.7/data/certs/default_cert.pem -subj "/O=INetSim/OU=Development/CN=inetsim.org" 2>/dev/null && echo "done." || echo "failed."
  else
...
chgrp -R inetsim inetsim-1.2.7/log
chmod 770 inetsim-1.2.7/log

chgrp inetsim inetsim-1.2.7/report
chmod 770 inetsim-1.2.7/report

chgrp -R inetsim inetsim-1.2.7/data
chmod g+w inetsim-1.2.7/data/http/postdata/
chmod g+w inetsim-1.2.7/data/ftp/upload/
chmod g+w inetsim-1.2.7/data/tftp/upload/
